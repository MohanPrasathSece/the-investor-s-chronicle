...
import { AlertCircle, CheckCircle, ArrowRight, Shield, Lock, Activity, KeyRound, Fingerprint, Radar, BadgeCheck } from "lucide-react";
...
export default function Enquiry() {
  ...
  useEffect(() => {
    // Update page title for enquiry page
    document.title = "Meridian Prime - Digital Asset Wealth Platform";
    const metaDesc = document.querySelector('meta[name="description"]');
    if (metaDesc) metaDesc.setAttribute("content", "Institutional-grade digital asset custody, multi-sig execution desks, and private wealth advisory solutions.");
  }, []);
  ...
  const validate = (): boolean => {
    ...
    if (!form.consent) errs.consent = "You must consent to be contacted";
    if (form.phone.trim()) {
                ...
                <form onSubmit={onSubmit} className="space-y-5" noValidate>
                  ...
                  <div className="grid sm:grid-cols-2 gap-5">
                    <Field label="Full Name" error={errors.name}>
                      <input type="text" value={form.name} onChange={setField("name")} placeholder="Your Name" className={inputCls} autoComplete="name" />
                    </Field>
                    <Field label="Email Address" error={errors.email}>
                      <input type="email" value={form.email} onChange={setField("email")} placeholder="your@email.com" className={inputCls} autoComplete="email" />
                    </Field>
                  </div>
                  ...
                  <div className="flex flex-col">
                    ...
                    <div className="flex gap-2">
                      <CountrySelect value={form.country} onChange={(c) => setForm((prev) => ({ ...prev, country: c }))} className="w-[105px] shrink-0" />
                      <input type="tel" value={form.phone} onChange={setField("phone")} placeholder={getCountry(form.country).placeholder} className={inputCls} autoComplete="tel" />
                    </div>
                    {errors.phone ? <span className="mt-1.5 block text-xs text-rose-400">{errors.phone}</span> : null}
                  </div>

                  <Field label="Message (Optional)" error={undefined}>
                    <textarea rows={3} value={form.message} onChange={setField("message")} className={`${inputCls} resize-y h-auto py-3`} placeholder="Write your message here..." />
                  </Field>
                  ...
                  <div className="pt-2">
                    ...
                    {errors.consent ? <p className="text-[10px] text-rose-400 mt-1.5 ml-8">{errors.consent}</p> : null}
                  </div>
...
function Field({ label, error, children }: { label: string; error?: string; children: React.ReactNode }) {
  return (
    <label className="block w-full">
      <span className="block text-[0.65rem] uppercase tracking-[0.1em] font-bold text-slate-400 mb-2">{label}</span>
      {children}
